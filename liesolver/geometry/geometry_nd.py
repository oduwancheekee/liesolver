...
class Hypercube(Geometry):
    ...
    def boundary_normal(self, x):
        _n = -isclose(x, self.xmin).astype(np.float32) + isclose(x, self.xmax)
    ...
    def uniform_points(self, n, boundary=True):
        ...
        for i in range(self.dim):
            ...
            if boundary:
                xi.append(
                    np.linspace(
                        self.xmin[i], self.xmax[i], num=ni, dtype=np.float32
                    )
                )
            else:
                xi.append(
    ...
    def uniform_boundary_points(self, n):
        ...
        for d in range(self.dim):
            for boundary in [self.xmin[d], self.xmax[d]]:
                ...
                for i in range(self.dim):
                    if i == d:
                        xi.append(np.array([boundary], dtype=np.float32))
                    else:
...
class Hypersphere(Geometry):
    ...
    def random_points(self, n, random="pseudo"):
        ...
        if random == "pseudo":
            ...
            X = np.random.normal(size=(n, self.dim)).astype(np.float32)
        else:
            rng = sample(n, self.dim + 1, random)
            U, X = rng[:, 0:1], rng[:, 1:]  # Error if X = [0, 0, ...]
            X = stats.norm.ppf(X).astype(np.float32)
    ...
    def random_boundary_points(self, n, random="pseudo"):
        ...
        if random == "pseudo":
            ...
            X = stats.norm.ppf(U).astype(np.float32)
        X = preprocessing.normalize(X)
        return self.radius * X + self.center
    ...
    def background_points(self, x, dirn, dist2npt, shift):
        ...
        pts = (
            x
            - np.arange(-shift, n - shift + 1, dtype=np.float32)[:, None]
            * h
            * dirn
        )
        return pts
