...
class GeometryXTime:
    ...
    def equal_split_uniform_points(self, n, boundary=True):
    ...
    def uniform_initial_points(self, n):
        ...
        return np.hstack((x, np.full([len(x), 1], t, dtype=np.float32)))

    def random_initial_points(self, n, random="pseudo"):
        x = self.geometry.random_points(n, random=random)
        t = self.timedomain.t0
        return np.hstack((x, np.full([n, 1], t, dtype=np.float32)))
